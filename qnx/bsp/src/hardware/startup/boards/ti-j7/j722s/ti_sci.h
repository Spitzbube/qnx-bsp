...
/*USB0 Main Power Control*/
#define TI_SCI_MSG_SET_MAIN_USB0    0x0402
...
int ti_sci_cmd_get_revision(struct ti_sci_msg_resp_version *revinfo);
int ti_sci_cmd_clk_get_freq(uint32_t dev_id, uint32_t clk_id, uint64_t *freq);
int ti_sci_cmd_set_clk_parent(uint32_t dev_id, uint32_t clk_id, uint32_t parent);
int ti_sci_cmd_clk_set_freq(uint32_t dev_id, uint32_t clk_id, uint64_t min_freq, uint64_t target_freq, uint64_t max_freq);
int ti_sci_cmd_proc_get_status(uint8_t proc_id, uint64_t *bv, uint32_t *cfg_flags, uint32_t *ctrl_flags, uint32_t *sts_flags);
int ti_sci_set_device_state(uint32_t id, uint32_t flags, uint8_t state);
int ti_sci_get_device_state(uint32_t id,  uint32_t *clcnt,  uint32_t *resets, uint8_t *p_state,  uint8_t *c_state);
int ti_sci_set_clock_state(uint32_t dev_id, uint32_t clk_id, uint32_t flags, uint8_t state);
int ti_sci_set_device_resets(uint32_t id, uint32_t reset_state);
int ti_sci_cmd_get_clock_state(uint32_t dev_id, uint32_t clk_id, uint8_t *programmed_state, uint8_t *current_state);

void * ti_sci_init(void);
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/startup/boards/ti-j7/j722s/ti_sci.h $ $Rev: 994584 $")
#endif
