...
#define SPROXY_RX_CHANNEL               11
#define SPROXY_TX_CHANNEL               12
...
static ti_sci_t ti_sci = {
    .scfg_addr        = 0x32800000,
    .rt_addr          = 0x32400000,
    .target_data_addr = 0x32C00000,
...
static int ti_sci_send_message(const void *const data, const int tlen)
{
 ...
 * ti_sci_get_device_state() - Get device state helper
 * @handle: Handle to the device
 * @msmc_start_low:  Pointer to Low 32-bits of physical address start of MSMC memory
 * @msmc_start_high: pointer to High 32-bits of physical address start of MSMC memory
 * @msmc_end_low:    pointer to Low 32-bits of physical addressable address
 *            (byte aligned) of valid range of MSMC memory.
 * @msmc_end_high:    pointer to High 32-bits of physical addressable address
 *            (byte aligned) of valid range of MSMC memory.
 *
 * Return: 0 if all went fine, else return appropriate error.
 */
int ti_sci_get_msmc(uint32_t *msmc_start_low,  uint32_t *msmc_start_high,
            uint32_t *msmc_end_low,  uint32_t *msmc_end_high)
{
    const ti_sci_t    *const sci = &ti_sci;
    uint32_t    buf[(SPROXY_MAX_MESSAGE_SIZE + sizeof(uint32_t) - 1) / sizeof(uint32_t)];
    const struct ti_sci_msg_query_msmc_req *req;
    const struct ti_sci_msg_query_msmc_resp *resp;

    ti_sci_setup_message((void *)buf, (uint8_t)seq++,
            (uint8_t)sci->host_id, TISCI_MSG_QUERY_MSMC, TI_SCI_FLAG_REQ_ACK_ON_PROCESSED);

    req = (struct ti_sci_msg_query_msmc_req *)buf;

    if (ti_sci_do_xfer(buf, sizeof(*req), buf, sizeof(*resp)) != 0) {
        return -1;
    }

    resp = (struct ti_sci_msg_query_msmc_resp *)buf;
    if (resp->hdr.flags != TI_SCI_FLAG_RESP_GENERIC_ACK) {
        kprintf("%s: response NACK\n", __func__);
        return -1;
    }
    ...
    if (msmc_start_low) {
        *msmc_start_low = resp->msmc_start_low;
    }
    if (msmc_start_high) {
        *msmc_start_high = resp->msmc_start_high;
    }
    if (msmc_end_low) {
        *msmc_end_low = resp->msmc_end_low;
    }
    if (msmc_end_high) {
        *msmc_end_high = resp->msmc_end_high;
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/startup/boards/ti-j7/ti_sci.c $ $Rev: 984580 $")
#endif
