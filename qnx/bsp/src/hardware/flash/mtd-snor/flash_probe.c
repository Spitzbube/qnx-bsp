        ...
        for (cs = 0; cs < numc; cs++) {
            for (es = 0; es < SNOR_MAX_ERSCFG; es++) {
                if (unit_size == (1u << ctrl->chip[cs].blkers[es].blksz_pow2)) {
                    ...
                    // set erase region eid
                    ctrl->chip[cs].eid[0] = es;
                    snor_slogf(_SLOG_INFO, ctrl->verbosity, _SLOG_INFO, "(devf  t%d::%s:%d) eid %d", pthread_self(), __func__, __LINE__, ctrl->chip[cs].eid[0]);
