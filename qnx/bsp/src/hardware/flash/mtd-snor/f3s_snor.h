...
typedef struct _snor_cfg_t {
    ...
    uint8_t     stripe;     // stripe mode CS bit field
#define SNOR_CFGFLGS_DBOP   (1 << 0)    // double OP code
#define SNOR_CFGFLGS_DBIOP  (1 << 1)    // double inverted OP code (the second OP code is the inverse of the first one)
    uint8_t     cflgs;      // flags
} snor_cfg_t;
...
#define SNOR_OPFLGS_AWREN   (1 << 0)    // support auto WREN for page program
#define SNOR_OPFLGS_ASP     (1 << 1)    // support auto status polling for page program
...
#define SNOR_CFLG_DNLOCK        (1u <<  8)  // support dynamic lock
    ...
    uint8_t     sp_op;      // status polling op code
    uint8_t     sp_bit;     // status polling bit
    uint8_t     sp_pol;     // status polling polarity

#define RDR_DC_DFLT     (0) // default read register dummy cycles
#define RDR_DC_OCTAL    (8) // read register dummy cycles in octal mode
    uint8_t     rdr_dc;     // read register dummy cycles
    ...
    uint8_t     vid;
    uint8_t     did;
#define SNOR_NO_DRVSTRG_OPT (0xFFFF)  // default drv_type vaule
    uint16_t    drv_type;   // output driver strength
...
#define SNOR_MAX_GEONUM     (8U)            // should be defined in fs-flash3 public header
    uint8_t     eid[SNOR_MAX_GEONUM];       // index in blkers array for each geometry
...
extern int     snor_read_sr(snor_chip_t* const chip, uint8_t *const sr, const int nsr);
...
extern int32_t f3s_mx25uw_ident(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t cs);
extern void f3s_mx25uw_reset(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset);

// Hyper flash
extern int32_t f3s_hpf_ident(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t cs);
extern void f3s_hpf_reset(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset);
extern int32_t f3s_hpf_sync(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset);
extern int f3s_hpf_erase(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset);
extern int32_t f3s_hpf_suspend(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset);
extern int32_t f3s_hpf_resume(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset);
extern int32_t f3s_hpf_program(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset, int32_t size, uint8_t *buffer);
