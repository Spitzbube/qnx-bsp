...
#define MT25Q_DRVSTRG_MSK    (0x7)    // Output driver strength mask
#define MT25Q_DIS_DTR        (1 << 5) // 1: Disable DTR
#define MT25Q_DIS_DUAL       (1 << 6) // 1: Disable Dual
#define MT25Q_DIS_QUAD       (1 << 7) // 1: Disable Quad
...
static int _mt25q_set_protocol(struct _snor_chip_t* const chip, const uint32_t proto)
{
    ...
    /* Bus mode */
    reg = (uint8_t)(evcr[0] | (MT25Q_DIS_QUAD | MT25Q_DIS_DUAL | MT25Q_DIS_DTR));
    ...
    if (chip->drv_type != SNOR_NO_DRVSTRG_OPT) {
        if (chip->drv_type > MT25Q_DRVSTRG_MSK) {
            ...
            switch (chip->drv_type) {
                ...
                case 0x7:
                    ...
                    if ((uint8_t)chip->drv_type != (uint8_t)(evcr[0] & MT25Q_DRVSTRG_MSK)) {
                        reg &= ~MT25Q_DRVSTRG_MSK;
                        reg |= chip->drv_type;
                    }
                    break;
