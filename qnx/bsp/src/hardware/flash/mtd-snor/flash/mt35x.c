...
#define MT35X_OP_RVCR       0x85    // read volatile configuration register
#define MT35X_OP_WVCR       0x81    // write volatile configuration register
#define MT35X_VCR_DRVSTRG   0x3     // driver strength register address
...
static int _mt35x_set_protocol(struct _snor_chip_t* const chip, const uint32_t proto)
{
    ...
    if (chip->drv_type != SNOR_NO_DRVSTRG_OPT) {
        switch (chip->drv_type) {
            /* Validate value */
            case 0xFF:
            case 0xFE:
            case 0xFC:
            case 0xFD:
                /* Get current driver strength */
                const uint8_t tmp = 0;
                int32_t err = snor_read_register(chip, MT35X_OP_RVCR, MT35X_VCR_DRVSTRG, alen, (uint8_t *)&tmp, sizeof(tmp));
                if (err != EOK) {
                    snor_slogf(_SLOG_ERROR, 0, 0, "%s: Read MT35X_VCR_DRVSTRG failed", __func__);
                    return (err);
                }
                /* Only update driver strength if it is different than current value */
                if (chip->drv_type != tmp) {
                    err = snor_write_register(chip, MT35X_OP_WVCR, MT35X_VCR_DRVSTRG, alen, (uint8_t *)&chip->drv_type, 1);
                    if (err != EOK) {
                        snor_slogf(_SLOG_ERROR, 0, 0, "%s: Write MT35X_VCR_DRVSTRG failed", __func__);
                        return (err);
                    }
                }
                break;
