...
int snor_post_ident(struct _snor_chip_t* const chip)
{
    ...
    if (chip->cfg.bus_proto == SNOR_BUSPROTO_HYPER) {
        /*
         * Everything is configured in hyper flash set_protocol callback
         */
        if (chip->set_protocol == NULL) return (EOK);
        if (chip->set_protocol(chip, SNOR_BUSPROTO_HYPER) != EOK) return (ENOTSUP);
    } else {
    ...
    /* Update page program auto WREN flag */
    if (ctrl->ccaps & SNOR_CCAPS_PPAWREN) {
        chip->op_wr.flags |= SNOR_OPFLGS_AWREN;
    }

    /* Update page program auto status polling */
    if (ctrl->ccaps & SNOR_CCAPS_PPASP) {
        chip->op_wr.flags |= SNOR_OPFLGS_ASP;
    }

    if (ctrl->verbosity > 2) {
        snor_slogf(_SLOG_INFO, ctrl->verbosity, 2,
                "(devf  t%d:%s bus[%x], read[bus-%x:op-0x%02X:alen-%d:dc-%d], write[bus-%x:op-0x%02X:alen-%d, rdr_dc:%d])",
                pthread_self(), __func__, chip->cfg.bus_proto,
                chip->rdcfg.bus_proto, chip->op_rd.opcode, chip->op_rd.adrlen, chip->op_rd.dcycle,
                chip->wrcfg.bus_proto, chip->op_wr.opcode, chip->op_rd.adrlen, chip->rdr_dc);
    }
...
static int snor_enter_quad(struct _snor_chip_t* const chip)
{
    ...
    if (chip->qes & (1u << 0)) {
         ...
         */
        if (snor_read_register(chip, 0x65, 0x800003, 3, &reg, 1) != EOK) return (EIO);
        reg |= (1u << 6);
        return snor_write_register(chip, 0x71, 0x800003, 3, &reg, 1);
    } else if (chip->qes & (1u << 4)) {
        /* 4S-4S-4S mode enable sequences:
