...
int32_t f3s_snor_sync(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t offset)
{
    ...
    nsr = (nsr > SNOR_MAX_CS) ? SNOR_MAX_CS : nsr;
    ...
    if (chip->dcaps & SNOR_DCAPS_FSTATUS) {
        ...
        for (i = 0; i < nsr; i++) {
            if (sr[i] & (SNOR_FLGSTS_EFAIL | SNOR_FLGSTS_PFAIL | SNOR_FLGSTS_EPROCT)) {
                if (snor_write_cmd(chip, SNOR_CMD_CLFSR) != EOK) {
                    snor_slogf(_SLOG_ERROR, 0, 0, "%s: clear flag status register failed", __func__);
                }
                return (EIO);
            }
    ...
    // use status register
    if (snor_read_sr(chip, sr, nsr) != EOK) return (EIO);
