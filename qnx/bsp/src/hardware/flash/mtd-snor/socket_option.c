                ...
                char *value, const int ws, const uint32_t lo, const uint32_t hi, const uint8_t nc)
{
    ...
    while (value != NULL) {
        ...
        if ((val > hi) || (val < lo)) {
            snor_slogf(_SLOG_ERROR, 0, 0, "%s: Invalid %s[0x%x]", fn, os, val);
            return (EINVAL);
        }
    ...
    while (options && (*options != '\0') && (status == EOK)) {
        ...
        switch (opt) {
            ...
            case OPTION_DRV_STRENGTH:       /* Driver strength */
                ...
                if (status == EOK) {
                    /* Clear higher byte. We only need lower byte which has stored OPTION_DRV_STRENGTH */
                    ctrl->chip[0].drv_type &= 0xFF;
                }
                break;
