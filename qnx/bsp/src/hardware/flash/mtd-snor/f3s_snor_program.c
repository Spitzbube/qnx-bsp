                         ...
                         uint8_t *buffer)
{
    ...
    // hardware can't automatically issue WREN command
    if ((!(chip->op_wr.flags & (uint8_t)SNOR_CCAPS_PPAWREN)) && (!(chip->cfg.bus_proto & SNOR_BUSPROTO_HPF_MODE))) {
    ...
    // always poll status register
    // no page program should exceed 100ms
    tmo = (chip->pptt && chip->ppmt) ? (((uint32_t)chip->pptt * (uint32_t)chip->ppmt) * 2) : 100000;
    tmo *= 10;
    while (--tmo) {
    ...
    if (!(flags & F3S_VERIFY_WRITE)) {
        size -= (pads + ((size < wrlen) ? 0 : tail));   // Can't simply set tail as "0"?
        if (tmpbuf != NULL) {
            free(tmpbuf);
        }
        return (size);
