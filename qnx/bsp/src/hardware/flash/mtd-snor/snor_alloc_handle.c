...
void *snor_alloc_handle(f3s_socket_t* const socket, const size_t nbytes)
{
    ...
    for (int i = 0; i < SNOR_MAX_CS; i++) {
        ...
        ctrl->chip[i].e2sl          = 150 * 1000;   /* Default erase to suspend lantency 150 us */
        ctrl->chip[i].er2si         = 150;          /* Default erase resume to suspend interval 150 us */
        ctrl->chip[i].sp_op         = SNOR_CMD_RDSR;/* Status polling op code */
        ctrl->chip[i].sp_bit        = 0;            /* Status polling bit */
        ctrl->chip[i].sp_pol        = 0;            /* Status polling polarity */
        ctrl->chip[i].drv_type      = SNOR_NO_DRVSTRG_OPT;  /* Output driver strength */
    }
