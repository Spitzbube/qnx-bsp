    ...
    )
{
    ...
    if (NULL == param || NULL == config) {
        ...
        param->wtlEnable       = (int)(config->enableWTL);
...
int openRoiMapFile(TestEncConfig *encConfig)
{
    if (encConfig->roi_enable) {
        ChangePathStyle((char *)encConfig->roi_file_name);
        if ((encConfig->roi_file = osal_fopen((char *)encConfig->roi_file_name, "r")) == NULL) {
            VLOG(ERR, "fail to open ROI file, %s\n", (char *)encConfig->roi_file_name);
            return FALSE;
        }
    }
    return TRUE;
