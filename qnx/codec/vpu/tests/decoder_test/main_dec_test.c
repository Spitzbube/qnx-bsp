...
static BOOL CheckTestConfig(TestDecConfig testConfig) {
    ...
    if ((FORMAT_224 <= testConfig.wtlFormat && testConfig.wtlFormat <= FORMAT_400) ||
        testConfig.wtlFormat < FORMAT_420 || testConfig.wtlFormat > FORMAT_422_P10_32BIT_LSB) {
...
static BOOL ParseArgumentAndSetTestConfig(CommandLineArgument argument, TestDecConfig* testConfig)
{
    ...
    struct OptionExt options_help[MAX_GETOPT_OPTIONS] = {
        ...
        {"codec",                 1, NULL, 0, "--codec                     codec index (AVC = 0, HEVC = 12)\n"},
        ...
        {"enable-uvintlv",        0, NULL, 0, "--enable-uvintlv            enable UV (CbCr) Interleave for NV12 support, default off\n"},
        ...
        {"annexb",                1, NULL, 0, "--annexb                    temp parameter to seperate out av1 stream format. 1: annex 0 : obu\n"},
        {NULL,                    0, NULL, 0},
    };
    ...
    while ((opt=getopt_long(argc, argv, optString, options, (int *)&index)) != -1) {
        ...
        case 0:
            if (!strcmp(options[index].name, "output")) {
            ...
            } else if (!strcmp(options[index].name, "enable-uvintlv")) {
                ...
                if (TRUE != testConfig->wave.av1Format) {
                    testConfig->wave.av1Format = 1; //AV1_STREAM_NOT_ANNEXB , extension .obu
                } else {
                    testConfig->wave.av1Format = 2; //AV1_STREAM_ANNEXB , extension .av1
                }
