...
int InitLog()
{
    fpLog = osal_fopen("/tmp/VPULog.txt", "w");
#if defined(SUPPORT_SW_UART) || defined(SUPPORT_SW_UART_V2)
...
void * osal_map_memory(Uint64 phys_addr, Uint32 len)
{
    void *va;

    va = mmap_device_memory(NULL, (size_t)len,
                    PROT_READ | PROT_WRITE | PROT_NOCACHE,
                            0, (uint64_t) phys_addr);
    if (va == MAP_FAILED)
    {
        printf("%s: mmap_device_memory failed for %lx, len = 0x%x\n",
...
int osal_munmap_memory(void *addr, Uint32 len)
{
...
int osal_get_interrupt_num(unsigned long core_idx)
{
#if defined(SOC_J784S4)
    if (core_idx == 1)
        return CSLR_COMPUTE_CLUSTER0_GIC500SS_SPI_CODEC1_VPU_WAVE521CL_INTR_0;
    else
        return CSLR_COMPUTE_CLUSTER0_GIC500SS_SPI_CODEC0_VPU_WAVE521CL_INTR_0;
#elif defined(SOC_J721S2) || defined(SOC_J722S)
    if (core_idx != 0)
        return -1;
#if defined(SOC_J721S2)
    return CSLR_COMPUTE_CLUSTER0_GIC500SS_SPI_K3_VPU_WAVE521CL0_VPU_WAVE521CL_INTR_0;
#elif defined(SOC_J722S)
    return CSLR_GICSS0_SPI_CODEC0_VPU_WAVE521CL_INTR_0;
#endif
#else // Unknown SoC
    codec_slogerr("%s:%d Unknown/New SOC. Interrupt num not identified\n", __FUNCTION__, __LINE__);
    return -1; // Invalid SOC core
#endif
}
...
osal_thread_t osal_thread_create(void(*start_routine)(void*), void*arg)
{
    ...
    if (thread == NULL) {
        codec_slogerr("%s:%d thread mem allocation failed\n", __FUNCTION__, __LINE__);
        return NULL;
    }
    if ((ret=pthread_create(thread, NULL, (void*(*)(void*))start_routine, arg)) != 0) {
        osal_free(thread);
        codec_slogerr("<%s:%d> Failure in pthread_create, ret(%d)\n", __FUNCTION__, __LINE__, ret);
    }
    else {
...
Int32 osal_thread_join(osal_thread_t thread, void** retval)
{
    ...
    if (thread == NULL) {
        codec_slogerr("%s:%d invalid thread handle\n", __FUNCTION__, __LINE__);
        return 2;
    }
    ...
    if ((ret=pthread_join(pthreadHandle, retval)) != 0) {
        osal_free(thread);
        codec_slogerr("%s:%d Failure in pthread_join, ret(%d)\n", __FUNCTION__, __LINE__, ret);
        return 2;
    }
...
Int32 osal_thread_timedjoin(osal_thread_t thread, void** retval, Uint32 millisecond)
{
    ...
    if ((ret=pthread_timedjoin(pthreadHandle, retval, &absTime)) == 0) {
        osal_free(thread);
        return 0;
    }
...
osal_mutex_t osal_mutex_create(void)
{
    ...
    if (mutex == NULL) {
        codec_slogerr("<%s:%d> failed to allocate memory\n", __FUNCTION__, __LINE__);
        return NULL;
    }

    if (pthread_mutex_init(mutex, NULL) < 0) {
        osal_free(mutex);
        codec_slogerr("<%s:%d> Failure in pthread_mutex_init(), errno(%d)\n", __FUNCTION__, __LINE__, errno);
...
void osal_mutex_destroy(osal_mutex_t mutex)
{
    ...
    if (mutex == NULL) {
        codec_slogerr("<%s:%d> Invalid mutex handle\n", __FUNCTION__, __LINE__);
        return;
    }

    if ((ret=pthread_mutex_destroy(mutex)) != 0) {
        codec_slogerr("<%s:%d> Failure in pthread_mutex_destroy(), ret(%d)\n", __FUNCTION__, __LINE__, ret);
    }
...
BOOL osal_mutex_lock(osal_mutex_t mutex)
{
    ...
    if ((ret=pthread_mutex_lock((pthread_mutex_t*)mutex)) != 0) {
        codec_slogerr("<%s:%d> Failure in pthread_mutex_lock(), ret(%d)\n", __FUNCTION__, __LINE__, ret);
...
BOOL osal_mutex_unlock(osal_mutex_t mutex)
{
    ...
    if ((ret=pthread_mutex_unlock((pthread_mutex_t*)mutex)) != 0) { //lint !e455
        codec_slogerr("<%s:%d> Failure in pthread_mutex_unlock(), ret(%d)\n", __FUNCTION__, __LINE__, ret);
        return FALSE;
    }
...
osal_sem_t osal_sem_init(Uint32 count)
{
    ...
    if (sem_init(semaphore, 0, count) < 0) {
        codec_slogerr("<%s:%d> Failed to init semaphore\n", __FUNCTION__, __LINE__);
        free(semaphore);
        semaphore = NULL;
    }
...
void osal_byte_swap(unsigned char* data, int len)
{
    Uint8 temp;
    Int32 i;
...
void osal_word_swap(unsigned char* data, int len)
{
    Uint16  temp;
...
void osal_dword_swap(unsigned char* data, int len)
{
    Uint32  temp;
...
void osal_lword_swap(unsigned char* data, int len)
{
    Uint64  temp;
