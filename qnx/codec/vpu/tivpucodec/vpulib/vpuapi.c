...
inline void VPU_SetPollingMode()
{
    codec_sloginfo("Forcing the lib to operate in polling mode. This is only for reference purposes");
    s_polling_mode = 1;
}
...
static RetCode InitializeVPU(Uint32 coreIdx, const Uint16* code, Uint32 size)
{
    ...
    if(s_polling_mode) { /* operate in polling mode */
        codec_sloginfo(" VPU registered in polling mode");
        vdi_set_polling(coreIdx);

    } else { /* operate in interrupt mode */
        if ((ret = vdi_interrupt_init(coreIdx)) != RETCODE_SUCCESS) {
            codec_slogerr(" interrupt init failed with error %d", ret);
            LeaveLock(coreIdx);
            return ret;
        }
    }
...
RetCode VPU_DecGetInitialInfo(DecHandle handle, DecInitialInfo* info)
{
    ...
    info->rdPtr = VpuReadReg(pCodecInst->coreIdx, pDecInfo->streamRdPtrRegAddr);
    info->wrPtr = VpuReadReg(pCodecInst->coreIdx, pDecInfo->streamWrPtrRegAddr);
    info->rdPtr = ((PhysicalAddress)vdi_get_axi_ext_addr() << 32) | info->rdPtr;
    info->wrPtr = ((PhysicalAddress)vdi_get_axi_ext_addr() << 32) | info->wrPtr;
...
RetCode VPU_DecGetBitstreamBuffer(DecHandle handle, PhysicalAddress* prdPtr, PhysicalAddress* pwrPtr, Uint32* size)
{
    ...
    if (pDecInfo->openParam.bitstreamMode != BS_MODE_PIC_END) {
        if (wrPtr < tempPtr) {
            ...
            room = (pDecInfo->streamBufEndAddr - wrPtr) + (tempPtr - pDecInfo->streamBufStartAddr);
        }
        room--;
    }
    else {
...
RetCode VPU_DecGetOutputInfo(DecHandle handle, DecOutputInfo* info)
{
    ...
    if (pDecInfo->mirrorEnable || pDecInfo->rotationEnable) {
    }
    else {
    ...
    case SET_SEC_AXI:
        {
            ...
            if (handle->productId == PRODUCT_ID_511 || handle->productId == PRODUCT_ID_517 || handle->productId == PRODUCT_ID_521) {
                ...
                pDecInfo->secAxiInfo.useLfRowEnable = secAxiUse.useLfRowEnable;
    ...
    case ENABLE_REP_USERDATA:
        {
            ...
            case PRODUCT_ID_517:
                ...
                break;
            default:
                codec_slogerr("%s(ENABLE_REP_DATA) invalid productId(%d)\n", __FUNCTION__, pCodecInst->productId);
                return RETCODE_INVALID_PARAM;
            }
            break;
        }
    ...
    int * size)
{
    ...
    if (GetPendingInst(pCodecInst->coreIdx) == pCodecInst) {
        /* NOTE: This path is never used for WAVE5 IPs (e.g., PRODUCT_ID_521) */
        wrPtr = ProductVpuEncGetWrPtr(pCodecInst);
    }
    else {
