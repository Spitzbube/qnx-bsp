...
Int32 WaveVpuGetProductId(Uint32  coreIdx)
{
    ...
    default:
        codec_slogerr("Check productId(%x)\n", val);
        break;
    }
    return productId;
}
...
static PhysicalAddress Wave5ReadRegForMemAddr(CodecInst* instance, Uint32 regAddr)
{
    PhysicalAddress addr = 0;
    Uint32 val;
    PhysicalAddress extAddr = 0;

    if (instance->isDecoder)
        extAddr = instance->CodecInfo->decInfo.openParam.priExtAddr;
    else
        extAddr = instance->CodecInfo->encInfo.openParam.priExtAddr;

    val = VpuReadReg(instance->coreIdx, regAddr);
    addr = ((extAddr << 32) | val);
    return addr;
}
...
RetCode Wave5VpuGetVersion(Uint32 coreIdx, Uint32* versionInfo, Uint32* revision)
{
    ...
    VpuWriteReg(coreIdx, W5_VPU_HOST_INT_REQ, 1);
    if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT, W5_VPU_BUSY_STATUS) == -1) {
        codec_slogerr("Wave5VpuGetVersion timeout\n");
        return RETCODE_VPU_RESPONSE_TIMEOUT;
    }

    if (VpuReadReg(coreIdx, W5_RET_SUCCESS) == FALSE) {
        codec_slogerr("Wave5VpuGetVersion FALSE\n");
...
RetCode Wave5VpuInit(Uint32 coreIdx, void* firmware, Uint32 size)
{
    ...
    vdi_fio_write_register(coreIdx, W5_BACKBONE_PROC_EXT_ADDR, vdi_get_axi_ext_addr());
    vdi_fio_write_register(coreIdx, W5_BACKBONE_AXI_PARAM, WAVE5_PROC_AXI_AXPROT<<4 | WAVE5_PROC_AXI_AXCACHE);
    VpuWriteReg(coreIdx, W5_SEC_AXI_PARAM, WAVE5_SEC_AXI_AXPROT<<20 | WAVE5_SEC_AXI_AXCACHE<<16 | WAVE5_SEC_AXI_EXT_ADDR);
    ...
    if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT, W5_VPU_BUSY_STATUS) == -1) {
        codec_slogerr("VPU init(W5_VPU_REMAP_CORE_START) timeout\n");
        return RETCODE_VPU_RESPONSE_TIMEOUT;
    }
    ...
    if (regVal == 0) {
        ...
        codec_slogerr("VPU init(W5_RET_SUCCESS) failed(%d) REASON CODE(%08x)\n", regVal, reasonCode);
        return RETCODE_FAILURE;
    }
...
RetCode Wave5VpuBuildUpDecParam(CodecInst* instance, DecOpenParam* param)
{
    ...
    VpuWriteReg(instance->coreIdx, W5_CMD_EXT_ADDR_BASE, param->priAxProt<<20 | param->priAxCache<<16 | param->priExtAddr);
...
RetCode Wave5VpuDecInitSeq(CodecInst* instance)
{
    ...
    if (VpuReadReg(instance->coreIdx, W5_RET_SUCCESS) == FALSE) {           // FAILED for adding a command into VCPU QUEUE
        ...
        if (regVal != WAVE5_SYSERR_QUEUEING_FAIL)
            codec_slogerr("FAIL_REASON = 0x%x\n", regVal);

        if (regVal == WAVE5_SYSERR_QUEUEING_FAIL) {
            regVal = VpuReadReg(instance->coreIdx, W5_RET_QUEUE_FAIL_REASON);
            codec_slogerr("QUEUE_FAIL_REASON = 0x%x\n", regVal);
...
RetCode Wave5VpuDecUpdateFramebuffer(CodecInst* inst, FrameBuffer* fbcFb, FrameBuffer* linearFb, Int32 mvIndex, Int32 picWidth, Int32 picHeight)
{
    ...
    codec_sloginfo("fbcIndex(%d), linearIndex(%d), mvIndex(%d)\n", fbcIndex, linearIndex, mvIndex);
...
RetCode Wave5VpuDecode(CodecInst* instance, DecParam* option)
{
    ...
    /* Secondary AXI */
    regVal = (pDecInfo->secAxiInfo.useBitEnable << 0)    |
             (pDecInfo->secAxiInfo.useIpEnable << 9)     |
             (pDecInfo->secAxiInfo.useLfRowEnable << 15);
...
RetCode Wave5VpuDecGetResult(CodecInst* instance, DecOutputInfo* result)
{
    ...
    result->bytePosFrameStart = Wave5ReadRegForMemAddr(instance, W5_RET_DEC_AU_START_POS);
    result->bytePosFrameEnd   = Wave5ReadRegForMemAddr(instance, W5_RET_DEC_AU_END_POS);
...
RetCode Wave5VpuReInit(Uint32 coreIdx, void* firmware, Uint32 size)
{
    ...
    oldCodeBase = ((PhysicalAddress)vdi_get_axi_ext_addr()<<32) | oldCodeBase;
    ...
    if (oldCodeBase != codeBase + W5_REMAP_INDEX1*W5_REMAP_MAX_SIZE) {
        ...
        if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT, W5_VPU_BUSY_STATUS) == -1) {
            codec_slogerr("VPU reinit(W5_VPU_REMAP_CORE_START) timeout\n");
            return RETCODE_VPU_RESPONSE_TIMEOUT;
        }
        ...
        if (regVal == 0) {
            Uint32 reasonCode = VpuReadReg(coreIdx, W5_RET_FAIL_REASON);
            codec_slogerr("VPU reinit(W5_RET_SUCCESS) failed(%d) REASON CODE(%08x)\n", regVal, reasonCode);
            return RETCODE_FAILURE;
        }
    }
...
RetCode Wave5VpuSleepWake(Uint32 coreIdx, int iSleepWake, const Uint16* code, Uint32 size)
{
    ...
    else //restore
    {
        ...
        if (vdi_wait_vpu_busy(coreIdx, __VPU_BUSY_TIMEOUT, W5_VPU_BUSY_STATUS) == -1) {
            codec_slogerr("VPU wakeup(W5_VPU_REMAP_CORE_START) timeout\n");
            return RETCODE_VPU_RESPONSE_TIMEOUT;
        }
        ...
        if (regVal == 0) {
            Uint32 reasonCode = VpuReadReg(coreIdx, W5_RET_FAIL_REASON);
            codec_slogerr("VPU wakeup(W5_RET_SUCCESS) failed(%d) REASON CODE(%08x)\n", regVal, reasonCode);
            return RETCODE_FAILURE;
        }
    }
...
RetCode Wave5DecClrDispFlag(CodecInst* instance, Uint32 index)
{
    ...
    if (ret != RETCODE_SUCCESS) {
        codec_slogerr("Wave5DecClrDispFlag QUERY FAILURE\n");
        return RETCODE_QUERY_FAILURE;
    }
...
RetCode Wave5VpuDecGetRdPtr(CodecInst* instance, PhysicalAddress *rdPtr)
{
    ...
    *rdPtr = Wave5ReadRegForMemAddr(instance, W5_RET_QUERY_DEC_BS_RD_PTR);
...
RetCode Wave5VpuEncGetRdWrPtr(CodecInst* instance, PhysicalAddress *rdPtr, PhysicalAddress *wrPtr)
{
    ...
    *rdPtr = Wave5ReadRegForMemAddr(instance, W5_RET_ENC_RD_PTR);
    *wrPtr = Wave5ReadRegForMemAddr(instance, W5_RET_ENC_WR_PTR);
...
RetCode Wave5VpuEncode(CodecInst* instance, EncParam* option)
{
    ...
    /* Secondary AXI */
    regVal  = (pEncInfo->secAxiInfo.useEncRdoEnable << 11) | (pEncInfo->secAxiInfo.useEncLfEnable << 15);
...
RetCode Wave5VpuEncGetResult(CodecInst* instance, EncOutputInfo* result)
{
    ...
    pEncInfo->streamWrPtr   = Wave5ReadRegForMemAddr(instance, pEncInfo->streamWrPtrRegAddr);
    pEncInfo->streamRdPtr   = Wave5ReadRegForMemAddr(instance, pEncInfo->streamRdPtrRegAddr);
    ...
    result->bitstreamBuffer = Wave5ReadRegForMemAddr(instance, pEncInfo->streamRdPtrRegAddr);
...
RetCode CheckEncCommonParamValid(EncOpenParam* pop)
{
    ...
    if (pop->bitstreamFormat == STD_HEVC) {
        if( !low_delay && (param->intraPeriod != 0) && (param->decodingRefreshType != 0) && (intra_period_gop_step_size != 0) &&
            (param->intraPeriod < intra_period_gop_step_size * 1)) {
                codec_slogerr("CFG FAIL : Not support intra period[%d] for the gop structure\n", param->intraPeriod);
                codec_slogerr("RECOMMEND CONFIG PARAMETER : Intra period >= %d\n", intra_period_gop_step_size * 2);
                ret = RETCODE_FAILURE;
        }
    }
    ...
    {
        codec_slogerr("CFG FAIL : Not support decoding refresh type[%d] for closed gop structure\n", param->decodingRefreshType );
        codec_slogerr("RECOMMEND CONFIG PARAMETER : Decoding refresh type = IDR\n");
        ret = RETCODE_FAILURE;
    }
    ...
    if (param->gopPresetIdx == PRESET_IDX_CUSTOM_GOP) {
        for(i = 0; i < param->gopParam.customGopSize; i++)
        {
            ...
            {
                codec_slogerr("CFG FAIL : temporalId %d exceeds MAX_NUM_TEMPORAL_LAYER\n", param->gopParam.picParam[i].temporalId);
                codec_slogerr("RECOMMEND CONFIG PARAMETER : Adjust temporal ID under MAX_NUM_TEMPORAL_LAYER(7) in GOP structure\n");
                ret = RETCODE_FAILURE;
            }

            if (param->gopParam.picParam[i].temporalId < 0)
            {
                codec_slogerr("CFG FAIL : Must be %d-th temporal_id >= 0\n",param->gopParam.picParam[i].temporalId);
                codec_slogerr("RECOMMEND CONFIG PARAMETER : Adjust temporal layer above '0' in GOP structure\n");
                ret = RETCODE_FAILURE;
            }
        }
    }
    ...
    // RDO
    {
        ...
        {
            codec_slogerr("CFG FAIL : Picture width and height must be aligned with 8 pixels when enable CU8x8 of cuSizeMode \n");
            codec_slogerr("RECOMMEND CONFIG PARAMETER : cuSizeMode |= 0x1 (CU8x8)\n");
            ret = RETCODE_FAILURE;
        }
        else if(((param->cuSizeMode&0x1) == 0) && ((param->cuSizeMode&0x2) == 0) && ((align_16_width_flag != 0) || (align_16_height_flag != 0)) )
        {
            codec_slogerr("CFG FAIL : Picture width and height must be aligned with 16 pixels when enable CU16x16 of cuSizeMode\n");
            codec_slogerr("RECOMMEND CONFIG PARAMETER : cuSizeMode |= 0x2 (CU16x16)\n");
            ret = RETCODE_FAILURE;
        }
        ...
        {
            codec_slogerr("CFG FAIL : Picture width and height must be aligned with 32 pixels when enable CU32x32 of cuSizeMode\n");
            codec_slogerr("RECOMMEND CONFIG PARAMETER : cuSizeMode |= 0x4 (CU32x32)\n");
            ret = RETCODE_FAILURE;
        }
    }
    ...
    // Slice
    {
        if ( param->wppEnable == 1 && param->independSliceMode == 1) {
            ...
            if (param->independSliceModeArg % num_ctb_in_width) {
                codec_slogerr("CFG FAIL : If WaveFrontSynchro(WPP) '1', the number of IndeSliceArg(ctb_num) must be multiple of num_ctb_in_width\n");
                codec_slogerr("RECOMMEND CONFIG PARAMETER : IndeSliceArg = num_ctb_in_width * a\n");
                ret = RETCODE_FAILURE;
            }
        }
        ...
        if(param->wppEnable == 1 && param->dependSliceMode != 0) {
            codec_slogerr("CFG FAIL : If WaveFrontSynchro(WPP) '1', the option of multi-slice must be '0'\n");
            codec_slogerr("RECOMMEND CONFIG PARAMETER : independSliceMode = 0, dependSliceMode = 0\n");
            ret = RETCODE_FAILURE;
        }

        if(param->independSliceMode == 0 && param->dependSliceMode != 0)
        {
            codec_slogerr("CFG FAIL : If independSliceMode is '0', dependSliceMode must be '0'\n");
            codec_slogerr("RECOMMEND CONFIG PARAMETER : independSliceMode = 1, independSliceModeArg = TotalCtuNum\n");
            ret = RETCODE_FAILURE;
        }
        else if((param->independSliceMode == 1) && (param->dependSliceMode == 1) )
        {
            ...
            {
                codec_slogerr("CFG FAIL : If independSliceMode & dependSliceMode is both '1' (multi-slice with ctu count), must be independSliceModeArg >= dependSliceModeArg\n");
                codec_slogerr("RECOMMEND CONFIG PARAMETER : dependSliceMode = 0\n");
                ret = RETCODE_FAILURE;
            }
        }
        ...
        if (param->independSliceMode != 0)
        {
            ...
            {
                codec_slogerr("CFG FAIL : If independSliceMode is not 0, must be independSliceModeArg <= 0xFFFF\n");
                ret = RETCODE_FAILURE;
            }
        }
        ...
        if (param->dependSliceMode != 0)
        {
            ...
            {
                codec_slogerr("CFG FAIL : If dependSliceMode is not 0, must be dependSliceModeArg <= 0xFFFF\n");
                ret = RETCODE_FAILURE;
            }
        }
    }
    ...
    if (param->confWinTop % 2) {
        codec_slogerr("CFG FAIL : conf_win_top : %d value is not available. The value should be equal to multiple of 2.\n", param->confWinTop);
        ret = RETCODE_FAILURE;
    }

    if (param->confWinBot % 2) {
        codec_slogerr("CFG FAIL : conf_win_bot : %d value is not available. The value should be equal to multiple of 2.\n", param->confWinBot);
        ret = RETCODE_FAILURE;
    }

    if (param->confWinLeft % 2) {
        codec_slogerr("CFG FAIL : conf_win_left : %d value is not available. The value should be equal to multiple of 2.\n", param->confWinLeft);
        ret = RETCODE_FAILURE;
    }

    if (param->confWinRight % 2) {
        codec_slogerr("CFG FAIL : conf_win_right : %d value is not available. The value should be equal to multiple of 2.\n", param->confWinRight);
        ret = RETCODE_FAILURE;
    }

    // RDO
    if (param->cuSizeMode == 0) {
        codec_slogerr("CFG FAIL :  EnCu8x8, EnCu16x16, and EnCu32x32 must be equal to 1 respectively.\n");
        ret = RETCODE_FAILURE;
    }

    if (param->losslessEnable && (param->nrYEnable || param->nrCbEnable || param->nrCrEnable)) {
        codec_slogerr("CFG FAIL : LosslessCoding and NoiseReduction (EnNrY, EnNrCb, and EnNrCr) cannot be used simultaneously.\n");
        ret = RETCODE_FAILURE;
    }

    if (param->losslessEnable && param->bgDetectEnable) {
        codec_slogerr("CFG FAIL : LosslessCoding and BgDetect cannot be used simultaneously.\n");
        ret = RETCODE_FAILURE;
    }

    if (param->losslessEnable && pop->rcEnable) {
        codec_slogerr("CFG FAIL : osslessCoding and RateControl cannot be used simultaneously.\n");
        ret = RETCODE_FAILURE;
    }

    if (param->losslessEnable && param->roiEnable) {
        codec_slogerr("CFG FAIL : LosslessCoding and Roi cannot be used simultaneously.\n");
        ret = RETCODE_FAILURE;
    }

    if (param->losslessEnable && !param->skipIntraTrans) {
        codec_slogerr("CFG FAIL : IntraTransSkip must be enabled when LosslessCoding is enabled.\n");
        ret = RETCODE_FAILURE;
    }
    ...
    // Intra refresh
    {
        ...
        {
            codec_slogerr("CFG FAIL : IntraCtuRefreshArg must be greater then 0 when IntraCtuRefreshMode is enabled.\n");
            ret = RETCODE_FAILURE;
        }
        if(param->intraRefreshMode == 1 && param->intraRefreshArg > num_ctu_row)
        {
            codec_slogerr("CFG FAIL : IntraCtuRefreshArg must be less then the number of CTUs in a row when IntraCtuRefreshMode is equal to 1.\n");
            ret = RETCODE_FAILURE;
        }
        if(param->intraRefreshMode == 2 && param->intraRefreshArg > num_ctu_col)
        {
            codec_slogerr("CFG FAIL : IntraCtuRefreshArg must be less then the number of CTUs in a column when IntraCtuRefreshMode is equal to 2.\n");
            ret = RETCODE_FAILURE;
        }
        if(param->intraRefreshMode == 3 && param->intraRefreshArg > num_ctu_row*num_ctu_col)
        {
            codec_slogerr("CFG FAIL : IntraCtuRefreshArg must be less then the number of CTUs in a picture when IntraCtuRefreshMode is equal to 3.\n");
            ret = RETCODE_FAILURE;
        }
        if(param->intraRefreshMode == 4 && param->intraRefreshArg > num_ctu_row*num_ctu_col)
        {
            codec_slogerr("CFG FAIL : IntraCtuRefreshArg must be less then the number of CTUs in a picture when IntraCtuRefreshMode is equal to 4.\n");
            ret = RETCODE_FAILURE;
        }
        if(param->intraRefreshMode == 4 && param->losslessEnable)
        {
            codec_slogerr("CFG FAIL : LosslessCoding and IntraCtuRefreshMode (4) cannot be used simultaneously.\n");
            ret = RETCODE_FAILURE;
        }
        if(param->intraRefreshMode == 4 && param->roiEnable)
        {
            codec_slogerr("CFG FAIL : Roi and IntraCtuRefreshMode (4) cannot be used simultaneously.\n");
            ret = RETCODE_FAILURE;
        }
    }
    return ret;
...
RetCode CheckEncRcParamValid(EncOpenParam* pop)
{
    ...
    if(pop->rcEnable == 1)
    {
        ...
        {
            codec_slogerr("CFG FAIL : Not allowed MinQP > MaxQP\n");
            codec_slogerr("RECOMMEND CONFIG PARAMETER : MinQP = MaxQP\n");
            ret = RETCODE_FAILURE;
        }

        if(pop->bitRate <= (int) pop->frameRateInfo)
        {
            codec_slogerr("CFG FAIL : Not allowed EncBitRate <= FrameRate\n");
            codec_slogerr("RECOMMEND CONFIG PARAMETER : EncBitRate = FrameRate * 10000\n");
            ret = RETCODE_FAILURE;
        }
    }
...
RetCode CheckEncCustomGopParamValid(EncOpenParam* pop)
{
    ...
    for(i = 0; i < gop_size; i++)
    {
        ...
        {
            codec_slogerr("CFG FAIL : the POC of the %d-th picture must be greater then -1\n", i+1);
            ret = RETCODE_FAILURE;
        }
        if(gopPicParam->pocOffset > gop_size)
        {
            codec_slogerr("CFG FAIL : the POC of the %d-th picture must be less then GopSize + 1\n", i+1);
            ret = RETCODE_FAILURE;
        }
        if(gopPicParam->temporalId < 0)
        {
            codec_slogerr("CFG FAIL : the temporal_id of the %d-th picture must be greater than -1\n", i+1);
            ret = RETCODE_FAILURE;
        }
    }
    ...
    for(ei = 1; ei < gop_size * 2 + 1; ei++)
    {
        ...
        if(new_gop[ei].picType != PIC_TYPE_I)
        {
            ...
            {
                codec_slogerr("CFG FAIL : the 1st reference picture cannot be used as the reference of the picture (POC %d) in encoding order\n", cur_pic->pocOffset - gop_size);
                ret = RETCODE_FAILURE;
            }
            if(enc_tid[ref_poc] > cur_pic->temporalId)
            {
                codec_slogerr("CFG FAIL : the temporal_id of the picture (POC %d) is wrong\n", cur_pic->pocOffset - gop_size);
                ret = RETCODE_FAILURE;
            }
            if(ref_poc >= cur_pic->pocOffset)
            {
                codec_slogerr("CFG FAIL : the POC of the 1st reference picture of %d-th picture is wrong\n", cur_pic->pocOffset - gop_size);
                ret = RETCODE_FAILURE;
            }
        }
        if(new_gop[ei].picType != PIC_TYPE_P)
        {
            ...
            if(enc_tid[ref_poc] < 0) // reference picture is not encoded yet
            {
                codec_slogerr("CFG FAIL : the 2nd reference picture cannot be used as the reference of the picture (POC %d) in encoding order\n", cur_pic->pocOffset - gop_size);
                ret = RETCODE_FAILURE;
            }
            if(enc_tid[ref_poc] > cur_pic->temporalId)
            {
                codec_slogerr("CFG FAIL : the temporal_id of %d-th picture is wrong\n", cur_pic->pocOffset - gop_size);
                ret = RETCODE_FAILURE;
            }
            if(new_gop[ei].picType == PIC_TYPE_P && new_gop[ei].useMultiRefP > 0)
            {
                if(ref_poc >= cur_pic->pocOffset)
                {
                    codec_slogerr("CFG FAIL : the POC of the 2nd reference picture of %d-th picture is wrong\n", cur_pic->pocOffset - gop_size);
                    ret = RETCODE_FAILURE;
                }
            }
