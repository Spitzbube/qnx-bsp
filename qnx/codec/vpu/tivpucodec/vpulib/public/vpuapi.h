...
/**
@verbatim
This command gets tiled map configuration according to `TiledMapConfig` structure. (TODO: remove)
...
/**
@verbatim
HOST can enable display buffer reordering when decoding H.264 streams. (WAVE decoder only)
...
*/
typedef struct {
    PhysicalAddress bufY;       /**< It indicates the base address for Y component in the physical address space when linear map is used. It is also compressed Y buffer or ARM compressed framebuffer (WAVE). */
    PhysicalAddress bufCb;      /**< It indicates the base address for Cb component in the physical address space when linear map is used. It is also compressed CbCr buffer (WAVE). */
    ...
    Uint32          bufYSize;   /**< It indicates the size for Y component in the physical address space when linear map is used. It is also compressed Y buffer size or ARM compressed framebuffer size (WAVE). */
    Uint32          bufCbSize;  /**< It indicates the size for Cb component in the physical address space when linear map is used. It is also compressed CbCr buffer size (WAVE). */
    Uint32          bufCrSize;  /**< It indicates the size for Cr component in the physical address space when linear map is used. */
/**
@verbatim
...
*/
    ...
    Uint32           priExtAddr;        /**< It indicates the high 16bit address of PRI_AXI. */
    Uint32           priAxProt;         /**< It indicates the AxProt for PRI_AXI. */
    Uint32           priAxCache;        /**< It indicates the AxCache for PRI_AXI. */
/**
@verbatim
...
/**
* @brief    This is a data structure for declaring SubFrameSync mode and interface in encoder (not actively configured for WAVE5)
*/
typedef struct {
/**
@verbatim
...
it completes a command or meets an exceptional case.
...
int VPU_GetFrameBufSize(
    ...
    DRAMConfig* pDramCfg    /**< [Input] Attributes of DRAM. Set NULL for this variable in case of Wave products. */
    );
...
/**
* @brief    This function forces the VPULib to operate in polling mode. By default, the lib is expected to operate in
interrupt mode.
*/
void VPU_SetPollingMode(void);
